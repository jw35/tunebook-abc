...
dist/tunebook.abc : abc/*.abc tunebook.template header.abc copying.abc
	mkdir -p dist
	./bin/joiner.py tunebook.template > $@
...
# and whistle tabs
dist/tunebook-tabs.pdf : abc/*.abc header.abc tabs.abc copying.abc tunebook.fmt flute.fmt guitarchords.fmt
	mkdir -p dist
	(echo '%abc-2.1'; \
